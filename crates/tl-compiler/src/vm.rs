...
impl Vm {
    pub fn new() -> Self {
        ...
        #[cfg(feature = "mcp")]
        {
            ...
            vm.globals
                .insert("mcp_ping".to_string(), VmValue::Builtin(BuiltinId::McpPing));
            vm.globals.insert(
                ...
                let contents: Vec<VmValue> = result
                    ...
                    .map(|content| {
                        ...
                        if let Some(uri_s) = json.get("uri").and_then(|v| v.as_str()) {
                            pairs.push((Arc::from("uri"), VmValue::String(Arc::from(uri_s))));
                        }
                        if let Some(mime) = json.get("mimeType").and_then(|v| v.as_str()) {
                            pairs.push((Arc::from("mime_type"), VmValue::String(Arc::from(mime))));
                        }
                        if let Some(text) = json.get("text").and_then(|v| v.as_str()) {
                            pairs.push((Arc::from("text"), VmValue::String(Arc::from(text))));
                        }
                        if let Some(blob) = json.get("blob").and_then(|v| v.as_str()) {
                            pairs.push((Arc::from("blob"), VmValue::String(Arc::from(blob))));
                        }
                        VmValue::Map(Box::new(pairs))
                    })
                    .collect();
                ...
                let messages: Vec<VmValue> = result
                    ...
                    .map(|m| {
                        ...
                        if let Some(role) = msg_json.get("role").and_then(|v| v.as_str()) {
                            msg_pairs.push((Arc::from("role"), VmValue::String(Arc::from(role))));
                        }
                        ...
                        if let Some(content) = msg_json.get("content") {
                            if let Some(text) = content.get("text").and_then(|v| v.as_str()) {
                                msg_pairs
                                    .push((Arc::from("content"), VmValue::String(Arc::from(text))));
                            } else {
    ...
    fn handle_train(
        ...
        for arg in &config_args {
            if let AstExpr::NamedArg { name, value } = arg {
                match name.as_str() {
                    ...
                    // n_estimators, alpha, k, eps, min_samples, ...). Previously
                    // dropped, so `train { n_trees: 80 }` silently used defaults.
                    other => match value.as_ref() {
                        AstExpr::Int(n) => {
        ...
        // Schema is shared across all batches.
        ...
        // Build the full row-major feature matrix + target across ALL batches.
        // Previously only batches[0] was used, so under DataFusion's parallel,
        // multi-partition output the model trained on a nondeterministic SUBSET of
        // the rows — producing flaky, sometimes-degenerate (all-one-class) models.
        ...
        for batch in &batches {
            ...
            for &ci in &feat_idx {
                ...
                Self::extract_f64_column(batch.column(ci), &mut tmp)?;
                cols.push(tmp);
            }
            for r in 0..nb {
                for col in &cols {
            ...
            Self::extract_f64_column(batch.column(target_idx), &mut target_data)?;
        }

        // Canonicalize row order so training is reproducible regardless of the
        // (nondeterministic) order DataFusion returns partitions/rows in.
        ...
        let target_tensor = tl_ai::TlTensor::from_vec(sorted_target, &[n_rows])
            .map_err(|e| runtime_err(format!("Shape error: {e}")))?;
